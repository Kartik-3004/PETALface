...
class Attention(nn.Module):
    ...
    def forward(self, x, alpha):
        with torch.cuda.amp.autocast(True):
            ...
            qkv = self.qkv(x, alpha.view(-1,1,1)).reshape(
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = Attention(lora_rank=self.lora_rank, lora_scale=self.lora_scale, dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, use_lora=self.use_lora)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(
            drop_path) if drop_path > 0. else nn.Identity()
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(lora_rank=self.lora_rank, lora_scale=self.lora_scale, in_features=dim, hidden_features=mlp_hidden_dim,
                       act_layer=act_layer, drop=drop, use_lora=self.use_lora)
        self.extra_gflops = (num_heads * patch_n * (dim//num_heads)*patch_n * 2) / (1000**3)

    def forward(self, x, alpha):
        x = x + self.drop_path(self.attn(self.norm1(x), alpha))
        with torch.cuda.amp.autocast(True):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList(
            [
                Block(lora_rank=self.lora_rank, lora_scale=self.lora_scale, dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                      drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                      num_patches=num_patches, patch_n=patch_n, use_lora=self.use_lora)
        ...
        if self.use_lora:
            self.feature = nn.Sequential(
                LoRALinear(in_features=embed_dim * num_patches, out_features=embed_dim, bias=False, r=self.lora_rank, scale=self.lora_scale),
                nn.BatchNorm1d(num_features=embed_dim, eps=2e-5),
                LoRALinear(in_features=embed_dim, out_features=num_classes, bias=False, r=self.lora_rank, scale=self.lora_scale),
